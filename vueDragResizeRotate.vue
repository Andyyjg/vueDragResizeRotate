<template>
    <div class="vdr" :style="{...style,background:bgColor,border:border,transform:'rotateZ('+deg+'deg)'}"
         :id="id"
         tabIndex=-1
        ...
        <span class="el-icon-refresh-right rotate" v-show="active && rotate"
...
<script>
    ...
    export default {
        ...
        mounted: function () {
            document.querySelector('.vdr').setAttribute("tabindex", 1) && document.querySelector('vdr').addEventListener("keyup", () => {
                alert(1)
            });
            this.cacuFather()
        ...
        methods: {
            ...
            getCenter() {
                ...
                this.rotateCenter = [el.getBoundingClientRect().left + el.offsetWidth / 2, el.getBoundingClientRect().top + el.offsetHeight / 2]
        ...
        computed: {
            style() {
                return {
                    ...
                    minHeight: this.height + 'px',//实现高度自适应
                    zIndex: this.zIndex
                }
            },
